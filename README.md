...
5. **QSIPrep**: Preprocesses diffusion MRI (DWI), including MP-PCA denoising and Gibbs unringing, running in parallel with FastSurfer.
...
Because Mindquad's dependencies are strictly defined, Snakemake inherently knows which steps can run simultaneously. Once BIDS conversion completes, **FastSurfer**, **QSIPrep**, and **MRS** run concurrently in parallel. Downstream pipelines (**fMRIPrep**, **HCP PostFreeSurfer**) wait for FastSurfer's structural reconstructions before launching.

If you supply enough cores (e.g., `mindquad -n 16`), or submit the pipeline to a SLURM cluster, Snakemake will automatically launch these independent pipelines at the exact same time!
